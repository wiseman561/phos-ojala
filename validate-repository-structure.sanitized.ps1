...
foreach ($category in $expectedStructure.Keys) {
    ...
    foreach ($path in $expectedStructure[$category]) {
        if (Test-Path $path) {
            ...
            Write-Host "  OK? MISSING: $path" -ForegroundColor Red
            $allGood = $false
        }
    }
}
...
foreach ($duplicate in $duplicateLocations) {
    if (Test-Path $duplicate) {
        Write-Host "  OK? DUPLICATE FOUND: $duplicate (should be removed)" -ForegroundColor Red
        $allGood = $false
    } else {
...
if (Test-Path $identityProjectPath) {
    ...
    foreach ($ref in $correctReferences) {
        if ($identityContent -match [regex]::Escape($ref)) {
            ...
            Write-Host "  OK? Missing correct reference: $ref" -ForegroundColor Red
            $allGood = $false
        }
    }
    ...
    foreach ($ref in $incorrectReferences) {
        if ($identityContent -match [regex]::Escape($ref)) {
            Write-Host "  OK? Incorrect reference found: $ref" -ForegroundColor Red
            $allGood = $false
        }
    }
} else {
    Write-Host "  OK? Identity project not found" -ForegroundColor Red
    $allGood = $false
}
...
foreach ($projectFile in $projectFiles) {
    if (Test-Path $projectFile) {
        ...
        if ($content -match "<TargetFramework>net8\.0</TargetFramework>") {
            ...
            Write-Host "  OK? $projectFile does not target .NET 8" -ForegroundColor Red
            $allGood = $false
        }
        ...
        if ($content -match "DisableImplicitNuGetFallbackFolder") {
            ...
            Write-Host "  WARN? $projectFile missing NuGet fallback folder setting" -ForegroundColor Yellow
        }
    }
}
...
if ($allGood) {
    ...
    Write-Host "OK? Repository structure has issues that need to be fixed." -ForegroundColor Red
